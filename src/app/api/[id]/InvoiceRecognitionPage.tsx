  ...
  const recognizeInvoice = async () => {
    ...
    try {
      const timeoutPromise = new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error('辨識超時')), 30000) // 30秒超時
      );
...
//     // 簡單的發票辨識邏輯（可根據實際需求優化）
//     const invoiceNumberMatch = text.match(/[A-Z]{2}-\d{8}/) || text.match(/\d{8}/);
//     const dateMatch = text.match(/\d{4}[/-]\d{2}[/-]\d{2}|\d{2}\/\d{2}\/\d{2}/);
//     const amountMatch = text.match(/金額[:：]?\s*(\d{1,3}(?:,\d{3})*|\d+)/);
//     const taxIdMatch = text.match(/統編[:：]?\s*(\d{8})/);

//     if (invoiceNumberMatch || dateMatch || amountMatch || taxIdMatch) {
//       setIsInvoice(true);
//       setInvoiceData({
//         invoiceNumber: invoiceNumberMatch ? invoiceNumberMatch[0] : '未找到',
//         date: dateMatch ? dateMatch[0] : '未找到',
...
//                 <Card sx={{ bgcolor: '#fff', borderRadius: '8px' }}>
//                   <CardMedia component="img" image={preview} alt="Uploaded" sx={{ height: 200, objectFit: 'contain' }} />
//                   <CardContent>
//                     <Button
//                       variant="contained"
//                       onClick={recognizeInvoice}
...
//                       sx={{ bgcolor: '#616161', '&:hover': { bgcolor: '#424242' }, color: 'white' }}
//                     >
//                       {isLoading ? <CircularProgress size={24} color="inherit" /> : '開始辨識'}
//                     </Button>
//                   </CardContent>
//                 </Card>
//               </Grid>
